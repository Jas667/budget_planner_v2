...
createNewCategoryButton.addEventListener('click', async () => {
    try {
        ...
        const results = await fetch('http://localhost:3000/categories', {method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({name: newCategoryNameValue})});
